...
void testEfficientFractPow(int N, int rep, unsigned NumDig, bool bRootOnly) {
    ...
    for (int j = 0; j < rep; ++j) {
        for (int i = 0; i < N*4; i+=4) {
            sum += EfficientFractPow(data1[i],   data2[i], NumDig); 
            sum += EfficientFractPow(data1[i+1], data2[i+1], NumDig); 
            sum += EfficientFractPow(data1[i+2], data2[i+2], NumDig); 
            sum += EfficientFractPow(data1[i+3], data2[i+3], NumDig); 
        }
    }
    timer.split();
    uint64_t t = timer.elapsed();
